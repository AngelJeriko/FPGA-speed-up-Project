...
#
# Do NOT launch the F1 until your AFI State.Code == "available" (poll from any box):
#   aws ec2 describe-fpga-images --fpga-image-ids <afi-...> \
#       --query 'FpgaImages[0].State.Code'
#
# Usage:
#   source sdk_setup.sh
#   scripts/f1/run_on_f1.sh -I agfi-0123456789abcdef [options]
#
# Options:
...
if [[ "$rc" -eq 0 ]]; then
  ...
  echo "    Bring back this log (and the Step-6 timing_summary) to close F1 bring-up."
else
...
echo "    Reminder: TERMINATE this f1.2xlarge when done — you only needed it for this run."
exit "$rc"
