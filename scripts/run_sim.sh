...
if [[ "$TB" == tb_msort ]]; then
...
elif [[ "$TB" == tb_chaining_extend_fetch ]]; then
    # A1 integration: same pipeline as tb_chaining_extend_top but the ref window comes from
    ...
    RTL_FILES=(
        ...
        "$RTL/ref_fetch_top.sv" "$RTL/chaining_extend_top.sv"
    )
    EO="$ROOT/host/extend_orchestrator"
    VEC_TXT="$EO/vectors/chainingext_vectors.txt"
    PLUSARGS=("+VEC=$VEC_TXT")
...
elif [[ "$TB" == tb_ref_fetch_top ]]; then
    RTL_FILES=(
        "$RTL/bsw_pkg.sv"
        "$RTL/ref_fetch_top.sv"
    )
    # self-contained tb (models HBM as g(addr)=addr&3); no vector bootstrap needed
    ...
    CH="$ROOT/host/chaining"
    VEC_TXT="$CH/vectors/chainingtop_vectors.txt"
    PLUSARGS=("+VEC=$VEC_TXT")
    if [[ ! -f "$VEC_TXT" ]]; then
        echo "Generating $VEC_TXT ..."
        mkdir -p "$CH/vectors"
        ( cd "$CH" && g++ -O2 -std=c++17 -o gen_chaining_top_vectors gen_chaining_top_vectors.cpp \
          && ./gen_chaining_top_vectors vectors/chainingtop_vectors.txt 4000 )
    fi
else
    RTL_FILES=(
        "$RTL/bsw_pkg.sv"
    ...
    if [[ "$TB" == tb_cl_bsw_ocl ]]; then
        RTL_FILES+=("$RTL/bsw_axil_regs.sv" "$RTL/f1/cl_bsw_top.sv")
    ...
    if [[ "$TB" == tb_cl_bsw_ocl_f2 ]]; then
        RTL_FILES+=("$RTL/bsw_axil_regs.sv" "$RTL/f2/cl_bsw_top.sv")
        VDEFINES+=("+define+CL_BSW_LINT")
    fi
