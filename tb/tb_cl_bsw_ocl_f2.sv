...
// -----------------------------------------------------------------------------
// Simulates the **F2** CL wrapper (rtl/f2/cl_bsw_top.sv) cl_bsw_top through its OCL AXI4-Lite port set (the
...
;
    ...
    // (F2 naming: ocl_cl_* = Shell->CL, cl_ocl_* = CL->Shell; addr padded to the 32-bit OCL width.)
    cl_bsw_top dut (
        .clk_main_a0(clk), .rst_main_n(rst_n),
        .ocl_cl_awaddr({16'b0, awaddr}), .ocl_cl_awuser('0), .ocl_cl_awvalid(awvalid), .cl_ocl_awready(awready),
        .ocl_cl_wdata(wdata), .ocl_cl_wstrb(wstrb), .ocl_cl_wvalid(wvalid), .cl_ocl_wready(wready),
        .cl_ocl_bresp(bresp), .cl_ocl_bvalid(bvalid), .ocl_cl_bready(bready),
        .ocl_cl_araddr({16'b0, araddr}), .ocl_cl_aruser('0), .ocl_cl_arvalid(arvalid), .cl_ocl_arready(arready),
        .cl_ocl_rdata(rdata), .cl_ocl_rresp(rresp), .cl_ocl_rvalid(rvalid), .ocl_cl_rready(rready)
    );
    ...
    initial begin
        ...
        $display("==== tb_cl_bsw_ocl_f2: %0d pass, %0d fail ====", pass, fail);
