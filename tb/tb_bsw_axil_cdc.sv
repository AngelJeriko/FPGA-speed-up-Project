// tb_bsw_axil_cdc.sv  (board-bringup track / F2 two-clock fallback)
// -----------------------------------------------------------------------------
// Same experiment as tb_bsw_axil, with the kernel moved to a SECOND CLOCK.
//
// The DUT is bsw_axil_regs with KERNEL_CDC=1, so bsw_top runs on clk_k behind
// bsw_kernel_cdc. The REF is still a bare bsw_top on clk. Both are given the same
// query/target/config and their results must be IDENTICAL -- so the property under
// test is exactly "putting the kernel in another clock domain does not change the
// engine's answer."
//
// The two clocks are deliberately hostile to the crossing:
//   * 10 ns vs 17 ns -- asynchronous and NON-HARMONIC, so the phase relationship
//     walks across every vector instead of repeating. A 2:1 ratio would let a broken
//     crossing pass by luck.
//   * clk_k starts 3 ns out of phase, and rst_k_n is released on a different edge
//     from rst_n. That reset skew is what the pending flags in bsw_kernel_cdc exist
//     to survive: a toggle that flips while the far synchroniser is still in reset
//     would otherwise be lost and deadlock the handshake.
//
// NOTE what a Verilator run can and cannot show. It proves the HANDSHAKE PROTOCOL is
// correct -- no lost requests, no lost results, no deadlock, no stale payload -- under
// a walking phase relationship. It does NOT model metastability; that is handled by
// the 2FF synchronisers and the XDC constraints, not by simulation.
// -----------------------------------------------------------------------------
`timescale 1ns/1ps
`include "bsw_pkg.sv"

module tb_bsw_axil_cdc
    import bsw_pkg::*;
;
    ...
    logic clk=0, rst_n=0;
    always #5 clk = ~clk;         // 100 MHz  -- AXI / front-end domain (stands in for
                                  //              F2's 250 MHz clk_main_a0)

    // Kernel domain: non-harmonic period and an initial phase offset, so the crossing
    // is exercised at a different alignment on every request.
    logic clk_k=0, rst_k_n=0;
    initial begin #3; forever #8.5 clk_k = ~clk_k; end   // 17 ns period, 3 ns skewed
    ...
    bsw_axil_regs #(.ADDR_W(ADDR_W), .DATA_W(DATA_W), .KERNEL_CDC(1'b1)) dut (
        .clk(clk), .rst_n(rst_n),
        .clk_k(clk_k), .rst_k_n(rst_k_n),
    ...
    initial begin
        ...
        // Reset skew on purpose: the kernel domain leaves reset well after the AXI
        // domain, so the handshake has to come up correctly from a staggered start.
        repeat (5) @(posedge clk); rst_n = 1; repeat (2) @(posedge clk);
        repeat (4) @(posedge clk_k); rst_k_n = 1; repeat (2) @(posedge clk_k);
        ...
        // 4-23. randomized small alignments (more than tb_bsw_axil: each one re-crosses
        // the domains at a fresh phase, so extra vectors buy real coverage here)
        ...
        $display("==== tb_bsw_axil_cdc: %0d pass, %0d fail ====", pass, fail);
