...
module tb_chaining_extend_fetch
    import bsw_pkg::*;
    import msort_v2_pkg::*;
();
    ...
    // ---- A1 on-chip fetch (replaces the host ref-server): ref_fetch_top reads the window from an
    //      HBM model over the ref_req/ref_in_* seam. The HBM model returns g(addr)=addr&3, the SAME
    //      bytes the host server streamed in tb_chaining_extend_top, so the pipeline result is
    //      unchanged IFF the on-chip fetch is bit-exact in-context. ----
    logic mem_arvalid; logic signed [63:0] mem_araddr; logic mem_arready; logic [7:0] mem_rdata; logic mem_rvalid;
    ref_fetch_top u_rf(.clk,.rst_n,
    ...
    initial begin
        ...
        $fclose(fd);
        $display("tb_chaining_extend_fetch: %0d cases, %0d failures -> %s", cnt, fails, (fails==0)?"ALL PASS":"FAIL");
        $finish;
    end
